...
The CLI functions can be accessed through `pyndex <command> ...`. The CLI follows a general `pyndex <command> <subcommand>` format, and each level's help documentation can be accessed with `--help`. The CLI stores local configuration data (connected indices, login info, etc) in the user's config directory by default, although other config files can be passed in as well.
...
Before deployment, the server requires a config file following the format outlined in [config.test.toml](config.test.toml), in a file named `config.toml` placed in the server's working directory. The server can then be run with `pyndex-server <options>`. Production deployment is WIP.
